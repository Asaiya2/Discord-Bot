...
class Utility(commands.Cog):
    ...
    async def cmdtest(self, ctx):
        ...
        if db_ok:
            try:
                conn = self.get_cached_connection(ctx.guild.id)
                c = conn.cursor()
                c.execute("SELECT COUNT(*) FROM reminders WHERE user_id = ?", (str(member.id),))
                remind_count = c.fetchone()[0]
                check("remind", True, f"{remind_count} pending reminder(s)")
            except Exception as e:
                check("remind", False, str(e))
    ...
    async def fullcmdtest(self, ctx):
        ...
        await test_command("remind", "10s", "Test reminder")
    ...
    async def remind(self, ctx, time_str: str, *, message: str):
        """Set a reminder. Usage: !remind 10m Check DMs"""
        if await self.check_duplicate(ctx):
            return
        ...
        try:
            ...
            if unit not in time_units:
                await ctx.send("❌ Invalid time format. Use s/m/h/d (e.g., 10m)")
                return
            ...
            c = conn.cursor()
            c.execute("INSERT INTO reminders (user_id, channel_id, message, remind_time) VALUES (?, ?, ?, ?)",
                      (str(ctx.author.id), str(ctx.channel.id), message, remind_time))
            reminder_id = c.lastrowid
            conn.commit()

            await ctx.send(f"✅ I'll remind you in {time_str}! (ID: {reminder_id})")

        except ValueError:
            await ctx.send("❌ Invalid time value. Use numbers (e.g., 10m)")
