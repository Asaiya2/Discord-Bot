...
class Tasks(commands.Cog):
    ...
    # ===== TASK: CHECK REMINDERS (Every 30 seconds) =====
    ...
    async def check_reminders(self):
        ...
        for guild in self.bot.guilds:
            try:
                ...
                c.execute("SELECT id, user_id, channel_id, message FROM reminders WHERE remind_time <= ?", 
                         (time.time(),))
                due = c.fetchall()
                ...
                print(f"Error checking reminders for guild {guild.id}: {e}")
